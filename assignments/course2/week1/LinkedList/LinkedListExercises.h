...
void LinkedList<T>::insertOrdered(const T& newData) {
 ...
 Node* nodenew = new Node(newData);
 Node* node_i = head_;

 size_ = size_ + 1;

  if(node_i == nullptr){// if linked list empty
    head_ = nodenew; // head node 
    tail_ = nodenew; // tail node
    return;
  }
  ...
  while(node_i != nullptr){
    ...
    Node* lastNode = node_i->prev;
    Node* nextNode = node_i->next;

    
    if((nodenew->data) < (node_i->data)){
      // if value less than head:
    ...
    if( !left.empty() && right.empty()) {
      while( !left.empty() ) {
        if( merged.empty() ) {
          ...
          const T last_val = merged.back();
          const T new_val = left.front();

          if(last_val < new_val) {
            merged.pushBack(new_val);
          }
          else {
            merged.popBack();
            merged.pushBack(new_val);
            merged.pushBack(last_val);
          }
        }
