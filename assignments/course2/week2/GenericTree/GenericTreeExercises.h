...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  // ...
    std::queue <TreeNode*> nodelevels;
    nodelevels.push(tree.getRootPtr());
    ...
    while (1) {
        ...
        qsiz = nodelevels.size();
        
        for (int m = 0; m < qsiz; ++m) {
            nodetemp=nodelevels.front();
            
            nodelevels.pop();
            
            vsiz = nodetemp->childrenPtrs.size();
            
            for (int k = 0; k < vsiz; ++k) { 
                results.push_back(nodetemp->childrenPtrs[k]->data);  
            
                nodelevels.push(nodetemp->childrenPtrs[k]);
            }
