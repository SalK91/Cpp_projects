...
int GridGraph::countEdges() const {
  ...
  for (auto &iter: adjacencyMap){
    numEdges = numEdges + iter.second.size();
  }
  return numEdges/2;
}
...
void GridGraph::removePoint(const IntPair& p1) {
  ...
  for (auto &kj: originalNeighbors){
    adjacencyMap[kj].erase(p1);
  }
  ...
  auto jk = adjacencyMap.begin();
  while(jk != adjacencyMap.end()){
    
    if(jk->first != p1){
      ++jk;
    } else{
      adjacencyMap.erase(jk);
      break;
    }
  }
